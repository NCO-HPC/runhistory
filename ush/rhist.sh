...
tarname=$(echo $tarname | perl -pe "s|lfs_[^_]+_ops_${envir}_(d?com)|\1|")
 ...
 then #
  ...
  #find $filepatterns \( -type l -o -type f \) | sort | uniq > $DATA/file.list
  eval "find $filepatterns \( -type l -o -type f \) | sort | uniq" > $DATA/file.list
 else #
  FILELIST=$(eval "find $filepatterns \( -type l -o -type f \) | grep -vFxf <(find $excludepatterns \( -type l -o -type f \)) | sort | uniq") #
  #find $filepatterns \( -type l -o -type f \) | grep -vFxf <(find $excludepatterns \( -type l -o -type f \)) | sort | uniq > $DATA/file.list
  eval "find $filepatterns \( -type l -o -type f \) | grep -vFxf <(find $excludepatterns \( -type l -o -type f \)) | sort | uniq " > $DATA/file.list
 fi #
...
#     cnt=`echo $FILELIST|wc -w`
#     cat $DATA/file.list|wc -l
#     getconf ARG_MAX
#     htar -Pcvf $fulltarpath $(echo $FILELIST)
      htar -Pcvf $fulltarpath -L $DATA/file.list
      status=$?
...
#added n in front of job GM
     if [ "$runjob" == "j${njob:1}" ]; then isrightjob=YES; fi
    else
#added n in front of job GM
     if [[ " ${runjobs[@]} " =~ " j${njob:1} " ]]; then isrightjob=YES; fi
    fi
    if [[ "$isrightjob" == YES ]]; then
#added n in front of job GM
     TRANSFERPARAMS[runjob]=j${njob:1}
     for whichpdy in ${whichpdys[@]}; do
